...
export const InfoCard = () => {
  ...
  const [showBuildInformation, setShowBuildInformation] = useState<boolean>(
    () =>
      localStorage.getItem('smartops-infocard-show-build-information') === 'true',
  );
  ...
  const toggleBuildInformation = () => {
    ...
    try {
      if (showBuildInformation) {
        localStorage.removeItem('smartops-infocard-show-build-information');
      } else {
        localStorage.setItem('smartops-infocard-show-build-information', 'true');
      }
    } catch (e) {
      // ignore
    }
  ...
  const filteredContent = () => {
    ...
    return buildDetails.filter(
      text =>
        text.startsWith('SmartOps Version') || text.startsWith('Backstage Version'),
    );
  };
