...
Examples of such a wrapper plugins can be found in the [Janus showcase repository](https://github.com/redhat-developer/rhdh/tree/main/dynamic-plugins/wrappers). For example, [backstage-community-plugin-scaffolder-backend-module-quay-dynamic](https://github.com/redhat-developer/rhdh/tree/main/dynamic-plugins/wrappers/backstage-community-plugin-scaffolder-backend-module-quay-dynamic) wraps the `@backstage-community/plugin-scaffolder-backend-module-quay` package to make it compatible with the dynamic plugin support. It then embeds the wrapped plugin code in the generated code and hoist its `@backstage` dependencies as peer dependencies in the resulting dynamic plugin through the use of the `--embed-package` option in the [`export-dynamic` script](https://github.com/redhat-developer/rhdh/blob/main/dynamic-plugins/wrappers/backstage-community-plugin-scaffolder-backend-module-quay-dynamic/package.json#L35).
